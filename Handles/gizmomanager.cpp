...
{
     OgreWidget *widget = MainWindow::getInstance()->ogreWindow;
...
void GizmoManager::ManipulationWithGizmo(Ogre::String gizmoName, Ogre::SceneNode* currentNode,float x,float y,bool localGizmo, bool globalGizmo)
{
   OgreWidget *widget = MainWindow::getInstance()->ogreWindow;
   ...
   Ogre::Vector3 intersection;
    ...
    if(gizmoName == "lineX" || gizmoName =="lineXCone" )
    {
        if(globalGizmo)
        {
            intersection=  RayManager::raycastIntersectionOnScene( x , y, OgreWidget::PLANE_XZ_MASK,false );
            ...
            UpdateAxisSize(translateGizmo,currentNode,gizmoName);
        }
        if(localGizmo)
        {
            intersection=  RayManager::raycastIntersectionOnScene(x , y, OgreWidget::PLANE_XZ_MASK,false,true,false,planeNode );
            Ogre::Vector3 deltaPos =intersection - startMousePosition;
    ...
    if(gizmoName == "lineY" || gizmoName =="lineYCone" )
    {
        if(globalGizmo)
        {
            intersection=  RayManager::raycastIntersectionOnScene( x , y, widget->PLANE_XY_MASK,false );
            Ogre::Vector3 deltaPos =intersection - startMousePosition;
        ...
        if(localGizmo)
        {
            intersection=  RayManager::raycastIntersectionOnScene(x , y, widget->PLANE_XY_MASK,false,true,false,planeNode );
            Ogre::Vector3 deltaPos =intersection - startMousePosition;
...
void GizmoManager::UpdateAxisSize(IGizmo* gizmo,Ogre::SceneNode* currentNode,Ogre::String gizmoName)
{
    OgreWidget *widget = MainWindow::getInstance()->ogreWindow;
